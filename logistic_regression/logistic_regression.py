...
def error_func(theta, X, y):
	return np.sum(-(y*np.log2(func_h(theta, X)) + (1-y) * np.log2(1-func_h(theta, X)) ), axis=1)

def cost_func(theta, X, y):
	return np.mean(error_func(theta, X, y), axis=0)

def gradient(theta, X, y):
	return X.T.dot(func_h(theta, X) - y) #* sigmoid(func_h(theta, X)) * (1-sigmoid(func_h(theta, X)))
